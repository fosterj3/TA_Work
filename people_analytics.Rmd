...
This exercise illustrates my ability to take raw data to address specific business questions related to people analytics and diversity, equity, and inclusion. I answer critical human resource (HR) questions, provide insights into recruitment and equity issues, and make recommendations using fabricated HR data. I manipulate, merge, visualize, and perform statistical tests on the HR data to answer business questions. The questions driving this analysis are: 
...
Based on this analysis, the executive team and directors should consider discontinuing to pay a search firm to help with recruitment. Costs can be reduced in this area since many of the "best" hires find the company online. The organization should reallocate the cost saved from not using a search firm to compensate women who perform the same duties as men fairly. Before new hires are onboarded, the hiring managers must review their compensation package to ensure new hires are getting paid equitably. Lastly, current employees must take advantage of their vacation days. Low engagement appears to be driven by departments whose employees take fewer vacation days. All work and no play make the employee disengaged.
...
kable(table, caption= "Average Attrition Percentage for Sales Team by Recruiting Source") %>% kable_styling(full_width = T, bootstrap_options = c("striped", "responsive"), position = "left", fixed_thead = T)
```
```{r}
# Visualizing Attrition Rates 
avg_attrition %>%
ggplot(aes(recruiting_source, attrition_rate )) + geom_col(aes(x = reorder(recruiting_source, attrition_rate)), fill = "black") + labs(x = "Recruiting Source", y = "Attrition Percentage", title = "Average Attrition Percentage by Recruiting Source") + scale_y_continuous(labels=scales::percent) + geom_text(aes(label = scales::percent(attrition_rate)), vjust = 1.5, colour = "white")
```
...
kable(table, caption= "Average Sales by Recruiting Source") %>% kable_styling(full_width = T, bootstrap_options = c("striped", "responsive"), position = "left", fixed_thead = T)
```
...
kable(table, caption= "Employee Engagement by Department") %>% kable_styling(full_width = T, bootstrap_options = c("striped", "responsive"), position = "left", fixed_thead = T) %>% footnote(general = " 1= low engagement, 5 = high engagement")
```
...
kable(table, caption= "Disengagement by Department") %>% kable_styling(full_width = T, bootstrap_options = c("striped", "responsive"), position = "left", fixed_thead = T) %>% footnote(general = "Disengagement is defined as having an engagement score of 1 or 2")
```
...
According to this model, new hires are not paid significantly more than current employees when accounting for job level. However, when also accounting for gender, they are paid more. 
```{r}
...
kable(table, caption= "Gender distribution of employees who received performance ratings", col.names = c("Gender", "Percent (Total)")) %>%
