...
2. Departments with the highest percentage of disengaged employees also have employees who have taken the fewest average vacation days
3. Pay between new hires and current employees is not equitable when accounting for gender
4. Women are less likely to be labeled high performers than men

# Executive Summary

This exercise aims to illustrate my ability to take raw data to address specific business questions related to people analytics and diversity, equity, and inclusion. I answer critical human resource (HR) questions, provide insights into recruitment and equity issues, and make recommendations using fabricated HR data. I manipulate, A visualization of, and perform statistical tests on HR data throughout this analysis. The questions driving this analysis are: 
...
The fabricated HR data is helpful to answer the questions above. However, the data is limited in terms of its demographic data. For example, this data does not contain age, race/ethnicity, ability, or education information. I can also not answer other crucial questions focused on legal and regulatory issues or effective diversity practices. However, this analysis suggests that people who applied online produced the best hires in terms of attrition and sales measures. On the other hand, the search firm produced the worse hires as measured by attrition and sales. 

Gallup defines engaged employees as involved in, enthusiastic about, and committed to their work and workplace. Gallup categorizes workers as "engaged" based on their responses to key workplace elements it has found predict important organizational performance outcomes. Employees in the sales department were more likely to report disengaging from work. Through careful analysis of the data, it was discovered that employees in the sales department took a significant amount of fewer vacation days, on average than the rest of the company. This is not to suggest not going on vacation causes disengagement; however, it may be beneficial to urge sales employees to use their vacation days. 

According to attorney Karen Denney, pay equity means compensating employees the same when they perform similar job duties while accounting for other factors, such as their experience level, job performance, and tenure. As a result, it is essential to disaggregate data based on race, gender, age, and other social axes of difference to determine whether an organization is paying its employees equitably. This analysis showed pay between new hires and current employees is not equitable when accounting for gender. Hiring managers and other key stakeholders should implement policies and practices to address this issue. The analysis revealed there are other existing inequitable issues regarding gender. Women are less likely to be labeled high performers than men; this may exacerbate pay gaps since performance ratings can affect bonuses and promotions. An initiative should be undertaken to ensure performance ratings are given fairly.

Based on this analysis, the executive team and directors should consider discontinuing to pay a search firm to help with recruitment. Costs can be reduced in this area since many of the "best" hires find the company online. Before new hires are onboarded, the hiring managers must review their compensation package to ensure new hires are getting paid equitably. Lastly, current employees must take advantage of their vacation days. Low engagement appears to be driven by departments whose employees take fewer vacation days. All work and no play make the employee disengaged. 
...
knitr::opts_chunk$set(echo=TRUE, message=FALSE, warning=FALSE, collapse = F, highlight = T, results = "asis")
...
table <- recruitment %>%
  ...
  summarize("Attrition Rate" = mean(attrition)) %>%
  adorn_totals("row") %>% 
  adorn_percentages(denominator = "col") %>%
  adorn_pct_formatting() %>%
  arrange(desc(`Attrition Rate`))

kable(table, caption= "Average Attrition Rate for Sales Team by Recruiting Source") %>% kable_styling(full_width = T, bootstrap_options = c("striped", "responsive"), position = "left", fixed_thead = T) %>% row_spec(c(5), bold = T)
```
...
ggplot(avg_sales, aes(recruiting_source, avg_sales)) + geom_col(aes(x = reorder(recruiting_source, avg_sales)), fill = "black") + labs(x = "Recruiting Source", y = "Average Sales", title = "Average Sales Rate by Recruiting Source")
...
avg_attrition %>%
ggplot(aes(recruiting_source, attrition_rate )) + geom_col(aes(x = reorder(recruiting_source, attrition_rate)), fill = "black") + labs(x = "Recruiting Source", y = "Attrition Rate", title = "Average Attrition Rate by Recruiting Source") + scale_y_continuous(labels=scales::percent)
```
...
kable(table, caption= "Employee Engagement by Department") %>% kable_styling(full_width = T, bootstrap_options = c("striped", "responsive"), position = "left", fixed_thead = T)
```
...
kable(table, caption= "Disengagement by Department") %>% kable_styling(full_width = T, bootstrap_options = c("striped", "responsive"), position = "left", fixed_thead = T)
```
...
ggplot(pay, aes(x = new_hire, fill = job_level)) + geom_bar(position = "fill") + scale_fill_manual(values = c("black", "darkred", "#1b9e77")) + labs(x = "New Hire", fill = "Job Level") + scale_y_continuous(labels=scales::percent) + labs(y = "Percent")
```
...
pay_grouped %>%
  ggplot(aes(new_hire, avg_salary)) + geom_col(aes(fill = job_level)) + facet_wrap(~job_level) + scale_fill_manual(values = c("black", "darkred", "#1b9e77")) + theme(legend.position = "none") + labs(x = "New Hire", y = "Average Salary", title = "New Hire's Average Salary by Job Level") + scale_y_continuous(labels=scales::dollar)
```

## Are new hourly employees getting paid more (statistically speaking)?
...
Here I use a multiple linear regression to test the difference in pay between new hires and current employees. The power of linear regression is that it can combine rigorous analysis to test the difference between groups with adding a filter (controlling for certain variables), and test again. By adding the additional variable directly into the regression, I get a significance result that takes additional information/variables into account.
```{r}
...
model_multiple %>% 
  tidy()
```
According to this model, new hires are not paid significantly more than current employees when accounting for job level. However, when controlling for job level, they are paid more. 
...
kable(table, caption= "Average Performance Rating by Gender") %>% kable_styling(full_width = F, bootstrap_options = c("striped", "responsive"), position = "left", fixed_thead = T) 
```
...
performance %>%
ggplot(aes(gender, fill = factor(rating))) + geom_bar(position = "dodge") + scale_fill_manual(values = c("#1b9e77","#d95f02","#7570b3","#e7298a","#66a61e")) + labs(title = "Distribution of Performance Rating by Gender", x = "Gender", fill = "Rating")
```
